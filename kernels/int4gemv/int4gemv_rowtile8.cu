// CUDALab INT4GEMV — INT4GEMV-0003 候选: x-stationary R=8 row tile
// （rowtile4 的 R 加倍: 每 block 8 行, x 片段跨 8 行复用）。
//
// 假设（来自 int4gemv_rowtile4 的 NCU @4096², clkbase）:
//   R=4 已命中 0002 假设: lg_throttle 43.9% -> 2.3% (0.22 cpi) 解决;
//   但 long_scoreboard 44.3% (4.31 cpi) 回升为第一停顿, DRAM 62.34%,
//   SM 46.95%, 占用率 62.83% (80 寄存器) —— 剩余瓶颈是 **W 侧 DRAM
//   延迟**: 每线程每 (v, r) 只有 1 条 16B W load 在飞, unroll R=4
//   后同时最多 4 条 W load 在飞 (MLP 4)。把行块从 4 行加到 8 行:
//     (1) 每线程同时最多 8 条独立 W load 在飞 (MLP 4 -> 8),
//         在飞 W 字节 2x, 应把 DRAM 从 62% 推向饱和并压低
//         long_scoreboard cpi;
//     (2) x 片段 8 行复用: 内存指令/行 384 -> 320 (x 侧 128 -> 64);
//     (3) scale 仍每行 1 次/向量 (行独立, 无压缩空间)。
//   代价: acc[8] + 可能整批驻留的 8 条 W uint4 (16 -> 32 寄存器),
//   寄存器 80 -> ~96-104, 占用率 62.83% -> 预期 ~45-50%。
//   若占用率塌方 (barrier/mio/no_instruction 上升、DRAM 不升反降),
//   则 R=8 为 REJECT 证据, 0004 转指令数压缩 (word-level unpack)。
//
// 结构（R=8, 与 rowtile4 唯一差异是 R 与 grid 粒度）:
//   grid  = ceil(N/8) 个 block, block = 128 线程
//   每 thread（对 K=4096 恰好 1 个 uint4 片段: K/32 = 128）:
//       strided uint4 循环 v = tid, tid+128, ...:
//         4× LDG.128 x 片段（k = 32v..32v+31）加载**一次**进寄存器,
//         转 float[32] 一次;
//         内层 unroll R=8: row = 8*blockIdx.x + i（guard row < N）:
//             1× LDG.128 W 向量 + 1× scale（单 group 引理 g = v>>2）
//             + 32 nibble unpack + 32×(MUL+FFMA) 进 acc[i];
//   归约: 8 个 acc[i] 各做 4-warp 归约（shfl 5 步 + shared[8][4] +
//         shfl 2 步）, warp 0 lane 0 写 out[row]。
//
// 数值: 每 thread 每 acc[i] 的 term 序列与 rowtile4/vec16_row 逐位
//   同序（v 升序, 向量内 k 升序, lo 先 hi 后）, 归约顺序相同 ——
//   预期与 rowtile4 bit-identical（层 A 硬门仍独立核验）。
//
...
//   的连续视图）不得被拒绝。
//
// 边界: N 不整除 8（如 N=1, N=3 的 edge 形状）时末 block 的部分
...
static constexpr int kRowTile8Block = 128;
static constexpr int kRowTile8Warps = kRowTile8Block / 32;  // 4
static constexpr int kRowTile8Rows = 8;

static __global__ void int4gemv_rowtile8_kernel(
    ...
    float acc[kRowTile8Rows];
#pragma unroll
    for (int i = 0; i < kRowTile8Rows; ++i) acc[i] = 0.f;
...
#pragma unroll
        for (int r = 0; r < kRowTile8Rows; ++r) {
            const int64_t row =
                    static_cast<int64_t>(kRowTile8Rows * blockIdx.x + r);
...
#pragma unroll
    ...
    __shared__ float warp_sums[kRowTile8Rows][kRowTile8Warps];
    if (lane == 0) {
#pragma unroll
        for (int r = 0; r < kRowTile8Rows; ++r) warp_sums[r][wid] = acc[r];
    }
    __syncthreads();
...
#pragma unroll
        for (int r = 0; r < kRowTile8Rows; ++r) {
            const int64_t row =
                    static_cast<int64_t>(kRowTile8Rows * blockIdx.x + r);
            if (row >= N) continue;
            float t = (lane < kRowTile8Warps) ? warp_sums[r][lane] : 0.f;
#pragma unroll
            for (int off = kRowTile8Warps / 2; off > 0; off >>= 1) {
...
void int4gemv_rowtile8_fwd(const at::Tensor& Wp, const at::Tensor& scale,
    ...
    const int grid = static_cast<int>((N + kRowTile8Rows - 1) / kRowTile8Rows);
    int4gemv_rowtile8_kernel<<<grid, kRowTile8Block, 0, stream>>>(
...
static struct Registrar {
    Registrar() { register_int4gemv_variant("int4gemv_rowtile8",
                                            int4gemv_rowtile8_fwd); }
} registrar;
