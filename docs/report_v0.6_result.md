...
baseline 84.96 µs（API-path, ~198 GB/s, 32% 峰值）→ 最终 incumbent
`qgemv_warp_vec16` 31.51 µs（533.4 GB/s 逻辑, 86.6% DRAM 峰值），
fresh revalidation paired 2.6937×（9/9 轮, CI95 [2.688, 2.700]）。
对比 v0.5 FP16 incumbent `gemv_vec4_row` 本 session 复测 59.93 µs：
INT8 1.90× 加速（理论 2.0× 的 95%）。**
...
- 三种计时口径（API-path / native kernel-loop w200+w5000 / NCU cc=all+cc=none
  @base clock）**分开记录、不混用**; 最终 incumbent 三口径 31.51 / 32.18–32.10
  / 36.16–35.88 µs — API 与 native 差 <2.5%, NCU 高 14.8% = NCU profiling
  固定开销（~4.4 µs 量级, 同 v0.5 ~3.8 µs）+ base clock 锁定贡献 ~0.8%
  （v0.5 clkbase/clknone 受控对, DRAM-bound kernel）, **无口径冲突**（§7）。
...
- CPU 测试基线: test_evaluator_v23_cpu 36/36、test_evaluator_cpu 18/18、
...
最终 incumbent fresh revalidation（9 轮 paired, streaming）✓; 三口径 ✓
...
第一杠杆。NCU 数字与 API/native 的差距由 NCU profiling 固定开销 +
base clock 锁定贡献（DRAM-bound 下 ~0.8%）解释（§7）。
...
- **0003 NEUTRAL（FASTER）**: warp-per-row 消掉 shared memory + barrier
  ...
  但 1.0107× 在 5% 政策带内 → 按政策 NEUTRAL（统计上显著更快, 见 §6 裁决）。
...
## 6. 最终 incumbent 选择（用户 §6/§7）

政策 incumbent 是 `qgemv_vec16_row`（唯一 KEEP）; `qgemv_warp_vec16`
统计显著更快但两轮都在 5% 政策带内（NEUTRAL）。最终裁决（fresh 证据驱动）:
...
- **裁决规则**: 两次独立 fresh pairing 的 CI95 均排除 1.00 且方向一致 →
  **最终 incumbent = `qgemv_warp_vec16`**（warp-per-row, ILP=2, 纯 warp
  shuffle, 无 shared/barrier; NCU 证据也最强: dram 87.24%, occ 90.43%,
  barrier stall 0）。`vec16_row` 保留为次优（结构更简, 差距 ~1%）。
  ilp4 出局（对 warp_vec16 统计 SLOWER）。

**最终 revalidation**（fresh 9 轮 paired, streaming, `final_incumbent_reval.json`）:
`qgemv_baseline` 84.885 µs → `qgemv_warp_vec16` **31.513 µs**（533.4 GB/s
逻辑 = 86.6% 峰值）→ **2.6937×**, CI95 [2.688, 2.700], 9/9 轮更快,
filter gate 无影响。
...
最终 incumbent `qgemv_warp_vec16` @4096×4096 fp16 streaming:
...
2. NCU 系统性高 ~14%, 两层分解: ① **base clock 锁定（1350 MHz）贡献
   ...
   ② **其余 ~13–14% 是 NCU profiling 固定开销**: 36.16 − 31.513 ≈ 4.65 µs
   （cc=none 亦 4.37 µs, +13.9%）—— 绝对量级与 v0.5 的 ~3.8 µs
   （~60 µs kernel 上的 6.4%）同档。注: 负载下实测 boost 为 1815–1920 MHz
   （repo 内 gpu_state 采样）, 早期稿「1545/1350 = 1.144 定量吻合」的归因
   **不成立, 已按独立 review（M1）更正**; 定性解释（锁频 + profiling
   开销只能让 NCU 更高, 方向正确）不变。cc=all 与 cc=none 差 0.8%
...
| QGEMV vec16_row（KEEP） | 31.808 µs | — | 36.37 µs (dram 86.0%) | 528.5 GB/s | 85.8% |
| **QGEMV warp_vec16（最终）** | **31.513 µs** | **32.103** | **36.16 µs (dram 87.24%)** | **533.4 GB/s** | **86.6%** |
...
**主结果: INT8 QGEMV 31.51 µs vs FP16 GEMV 59.93 µs = 1.90×**
（理论 2.0× 的 95%; 剩余 5% 来自 x/scale/y 的非减半流量与效率差
86.6% vs 90.9%）。QGEMV 最终 vs 其 baseline = 2.69×; vs FP16 baseline
（91.93 µs, 本 session 同口径复测）= 2.92×。
...
| (N, K) | mode | baseline | vec16_row | vec16_scale | **warp_vec16** | ilp4 | per-cell winner |
|---|---|---|---|---|---|---|---|
| (1024, 4096) | hot / streaming | 14.53 / 14.78 | 7.38 / 7.50 | 7.52 / 7.55 | **6.52 / 6.66** | 6.59 / 6.68 | warp_vec16 ×2 |
| (4096, 1024) | hot / streaming | 19.39 / 19.58 | 13.31 / 13.49 | 14.87 / 15.07 | **6.64 / 7.38** | 6.78 / 7.36 | warp_vec16 (hot, 1.022× CI 显著) / ilp4 名义 (streaming, CI 跨 1.0) |
...
**读法**:
1. **所有向量化变体在全部 10 格都是 baseline 的 2.2–2.9×**
   （winner-vs-baseline 实测区间 2.22–2.92×; (11008,4096) 格为
   2.821×/2.820×）; 含 11008 的长 K 格与主目标格内, 向量化变体之间差
   ≤~2%, 但短 1-dim 格差异是 regime 级的: (4096,1024) streaming 格
   vec16_row 13.49 vs warp_vec16 7.38 = **2.05×**（(1024,4096) 格也有
   ~15%）—— 长 K/多行 regime（覆盖主目标）下贴 DRAM 墙后 kernel 形态
   差异被抹平, 这解释了为什么 4 个实验里只有第一跳（向量化）产生
   2.66×, 后续轮次都在 ±1% 内; 形态差异只在短 K/短 N regime 显现（项 2–3）。
2. **warp_vec16 赢 6/10 格**, 包括主目标（两 mode）; 在 (4096, 1024) 短 K
   格对 vec16_row 有 **~2.0× 的结构性优势**: K=1024 → 每行只有 64 个
   16B 向量, block-per-row（256 线程）75% 线程空转, warp-per-row
   （32 线程 × ILP2 = 64 向量）恰好满负荷 —— 这是矩阵里唯一的 regime
   级差异, 也是「单一变体做最终 incumbent」时选 warp_vec16 的最强论据。
3. **vec16_row 赢 4/10 格** = 两个含 11008 的形状 × 两 mode, 领先
   0.4–0.6%（CI 均排除 1.0, 真实但小）: 长 K/多行 regime 下 block-per-row
   的 256 线程对 256 向量恰好满负荷, warp-per-row 的 8 warp 归约路径略逊。
4. **ilp4 无统计显著胜利**（1 格名义胜 CI 跨 1.0）—— 与 §5 QGEMV-0004
   一致: 更深的 ILP 不产生优势, 只在个别格与 tie 打平。
5. hot 与 streaming 结果逐格几乎重合（W ≫ L2, cache 状态不改变 regime）;
   (4096,1024) streaming 格 8/9 有效轮（1 环境无效轮, 已记录在 JSON）。
6. 11008 形状逻辑 BW ~560 GB/s（90.9% 峰值）—— 大 N 时 scale 表占比
   降低, 效率反超主目标的 86.6%; 主目标的 3.3% 差距 = scale (16 KB) 的
   固定额外 pass 占比更大。

**最终 incumbent 不变**: `qgemv_warp_vec16`（主目标两 mode 胜出 +
(4096,1024) 结构性优势 + 唯一无短 K 缺陷的变体 + NCU 证据最强; 在两个
11008 格落后 vec16_row 0.4–0.6%, 已在 winner 文件与本报告如实记录）。
...
| Finding | 内容 | 处置 |
|---|---|---|
| M1 (MAJOR) | 「boost 实测 ~1545 MHz / 1545÷1350=1.144」归因无据, 且与 repo 内 gpu_state 负载采样（1815–1920 MHz）及 v0.5 受控对矛盾 | **已修（本稿）**: §1/§7/§11 + README + STATUS 全部改为两层分解 —— 锁频贡献 ~0.8%（v0.5 DRAM-bound 受控对 64.192/63.704 = +0.77%）+ NCU profiling 固定开销 ~4.4–4.7 µs（同 v0.5 ~3.8 µs 量级）; 早期归因在文中明确标记为已更正。无决策影响（决策全在 API 口径内） |
...
kernel 正确性结论、统计决策或最终 incumbent 选择; benchmark 路全部
...
**1.90×**（31.51 µs vs 59.93 µs, API-path, 主目标, 同 session 复测）。
理论 2.0×（逻辑 IO 16.81 vs 33.575 MB）的 **95%**。相对各自 baseline:
QGEMV 2.69×（84.89 → 31.51）vs FP16 1.53×（91.93 → 59.93, 本 session）。
...
**`qgemv_warp_vec16`**（warp-per-row, ILP=2 双累加器, 纯 5 步 warp shuffle,
无 shared/barrier, 41 寄存器, occ 90.43%）。有效原因全部有 NCU 证据:
① 去掉 block 级 barrier（vec16_row barrier stall 7.3% → 0）;
② 占用率 82.6% → 90.43%（更多 warp 可掩盖延迟）;
③ dram 87.24% = 各变体最高。两次 fresh head-to-head 均统计显著优于
vec16_row（1.0094–1.0107×, CI95 排除 1.00）。更深 ILP（0004, ILP=4）
...
NCU 36.16 高 +14.8% = NCU profiling 固定开销（~4.4–4.7 µs, 同 v0.5 ~60 µs
kernel 上 ~3.8 µs 的量级）+ base clock 锁定贡献 ~0.8%（v0.5 clkbase/clknone
受控对: DRAM-bound +0.77% / latency-bound +15.9%, 本 kernel 属前者; 负载下
实测 boost 1815–1920 MHz, 早期稿「1545 MHz / 1.144 比值」归因已按 review
更正, M1）。cc=all vs cc=none 差 0.8%。所有口径排序一致, 任何口径选择
都不改变任何决策。
...
- profile: `profiles/qgemv/*`（baseline/vec16_row/vec16_scale/warp_vec16/
  warp_vec16_ilp4; 最终 incumbent 含 cc=all+cc=none）
