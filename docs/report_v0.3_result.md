...
4. **M 扩展（DRAM 饱和）**：(1024,4096) fp16 34.6 µs ≈ **485 GB/s**
   （16.78 MB/34.56 µs，≈ 88% 卡规格 550 GB/s）——工作集（8.4 MB/方向）
   超出 L2，进入带宽受限区，inc 与 base 差距收敛到 1.33–1.44×；(1024,4096)
   fp32 两变体均 ~93 µs（带宽墙，向量化不再显著）。
...
- **streaming 逻辑带宽可超过 DRAM 规格峰值**：16-buffer 轮转使 buffer 在
  5.5 MB L2 中部分驻留，`algorithmic_bw_gbps` 是逻辑流量代理而非实测
  DRAM 带宽；实测 DRAM 占比以 NCU 为准（主目标 30.9–36.2%）。
