...
| 三代对比（fresh 同 session） | @4096² streaming fp16，2026-09-22：FP16 `gemv_vec4_row` 59.977 / INT8 `qgemv_vec16_row` 31.931 / **INT4 hx 20.503 µs**（API；native 59.673/31.486/22.208；NCU 64.24/36.432/25.176 µs，dram 89.3/86.55/68.84%）。**INT4 vs INT8 = 1.557×（理论 2× 的 78%，未接近 2×）**；vs FP16 = 2.925×；INT8 vs FP16 = 1.878×（v0.6 结论 1.90× 的 fresh 复测确认）。缺口分解（因子分解 1.940× IO 减半 × 0.795× DRAM 效率 = 1.543× ≈ 实测）：（1）DRAM 效率 68.84% vs 86.55% 是大头（效率持平则 ~1.94× = 全逻辑 IO 理想值）；（2）实测 DRAM 流量超逻辑 18.4%（+1.6 MB：x 8 KB 被 1024 block 各读一次，W 流 8.4 MB > L2 5.5 MB streaming 逐出 x → DRAM 再取 ~196 量级，v0.8 可经 L2 访问策略攻击）；（3）整数线程指令 46.33M vs INT8 38.93M = +19%（每 product 2.76 vs 2.32，nibble 解包 + group scale 索引）；short_scoreboard 仅 4.5% → **group scale 查找不是瓶颈**（单 group 引理 g=v>>2） |
...
| git | 分支 `v0.7-int4-gemv`（10 commit）：2933d87（算子+量化+正确性+负例+baseline）→ bf6aabb（0001）→ 52856c6（0002）→ 5dbfeea（0003）→ fed572f（0004）→ 506da9a（全矩阵+winners）→ b2580d6（三代测量）→ c4aa04c（回归）→ (末) 最终报告+README/STATUS+两路 review 处置+gen3 pipe CSV 归档；**不 merge main、不 force push**，本 commit 后 push 即 STOP 等外部 review |
| 未做 / v0.8 backlog | 结构性 INT4 优化收尾（收益递减 2.0→1.188→1.087×，MLP 方向已证伪）；v0.8 候选（报告 §11 Q7）：（a）+18.4% DRAM 流量冗余（L2 访问策略窗口：W streaming / x 驻留，期望 5–10%）；（b）解包指令开销（编译器级，<5%）；（c）K=1024 线程利用率（128 线程中 96 空转，新结构）；**建议启动 CUDALM 集成**，以 `int4gemv_rowtile4_hx` 为交付内核 |
