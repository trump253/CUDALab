...
**日期：** 2026-09-22
**阶段：** v0.7 — W4A16 Group-wise INT4 GEMV（第六算子）
**状态：** 分支 `v0.7-int4-gemv`（基线 main = v0.6.1 = b0c5c7d = tag v0.6.1），本 commit 后 push 即 STOP 等外部 review，**不 merge main、不 force push**（用户指定）；停止条件全部达成：三层正确性（层 A kernel vs CPU 解包+反量化 FP32 参考 50/50 × 5 变体，固定算术界 TOL_K=2；层 B CPU 量化+pack nibble 互逆 39/39；层 C 量化保真度 report-only）+ per-variant 负例 30/30 × 5（含 2 例对齐回退 bit-identical 钉死）、三口径（API / native kernel-loop / NCU kernel duration）分开记录且排序在三口径间完全一致（API streaming 池 W L2-cold vs native 连续循环部分 L2-warm 的 regime 差已按 v0.5/v0.6 模式调查解释；NCU vs native 差 = observed profiling perturbation，本 workload ~3 µs 量级，不声称跨 workload 常数）、4 组预注册实验（INT4GEMV-0001..0004，3 KEEP + 1 NEUTRAL，rowtile8 失败结构归档保留）、全矩阵（hx 6/10 格 SIGNIFICANT_WINNER、9/10 格最低中位数，余 4 格 NO_UNIQUE_WINNER: (1024,4096) 打平 ×2 + (11008,4096) 子 5% ×2）、政策 incumbent **`int4gemv_rowtile4_hx`**（自 0004 KEEP 起）、三代同 session 对比（INT4 vs INT8 **1.557×** = 理论 2× 的 78%；vs FP16 **2.925×**；缺口两层视图：algorithmic 逻辑 IO 减半 1.940× / measured NCU 物理流量削减 1.796× × 物理带宽比 0.806× = NCU 1.447×，API 单独报告）、三算子回归 `experiments/regression/v0.7/` 全绿（int4gemv 5 + gemv 4 + qgemv 5 变体，隔离审计零泄漏）、最终报告 `docs/report_v0.7_result.md`（12 节，§10 含独立 review 结论与处置表，§11 用户 7 问）定稿；两个独立 subagent review（CUDA correctness + Benchmark methodology）结论与处置见报告 §10。**v0.7.1 merge 修复（外部 review 4 项）**: (1) 恢复历史 profile 不可变性——b2580d6 就地覆盖的 3 个 v0.5/v0.6 时代 profile JSON 恢复为 main 原版本，v0.7 fresh 三代 NCU 数据迁移至 `profiles/gen3_v0.7/`（append-only，5 JSON + README；规则 = historical artifacts immutable / fresh cross-generation measurements append-only）；(2) INT8→INT4 缺口分解改写为两层视图（§8: algorithmic 1.940× / measured NCU physical 1.7963××0.8056×=1.4471×，数字全部从 raw CSV/JSON 复算）；(3) 18.4% excess DRAM traffic 归因从结论降级为 hypothesis（x 跨 block 重读 / W streaming L2 再取假设，亦可能含 transaction 级流量，需控制实验）；(4) v0.8 backlog 移除当前 sm_75 不适用的 cudaAccessPolicyWindow/persisting-L2 方向，改为 Turing 兼容 cache 行为调查。不重跑 correctness/benchmark/NCU/matrix；kernel/experiment/benchmark raw JSON/gen3 CSV 均未改动。**不 merge main、不 force push。**

## v0.7 完成摘要（2026-09-22）

核心问题：**权重从 INT8 1B → INT4 0.5B（W4A16, G=128 group-wise, fp16 scale）后,
GEMV 能否拿到逻辑 IO 减半的 ~2× 收益? INT4 的 nibble unpack / group scale
查找 / 指令开销代价多大?** 最终报告：`docs/report_v0.7_result.md`（12 节，
§10 独立 review 处置，§11 用户 7 问，§12 产物索引）。

| 项目 | v0.7 结果 |
|---|---|
| INT4GEMV 算子 | `kernels/int4gemv/` 5 变体：`int4gemv_baseline`（标量 uint8 load，one row/block 256 线程）/ `int4gemv_vec16_row`（16B 向量化：U32I4×4 = 32 int4 + fp16 scale，1 row/block，每行重发 x）/ `int4gemv_rowtile4`（R=4 行块，x 片段 float 驻留 32 寄存器跨 4 行复用，4 独立 FMA 链）/ `int4gemv_rowtile8`（R=8，MLP 4→8，**NEUTRAL 归档**）/ **`int4gemv_rowtile4_hx`**（R=4 + x 片段 `__half2` 驻留 16 寄存器，逐行 `__half22float2` 转换，64 寄存器无 spill，**incumbent**）；`int4gemv_common.h` 单一来源标量 kernel（回退 bit-identical）+ 对齐契约 + 16B 解包原语。`W_packed uint8 (N,K/2)` + `scale fp16 (N,K/128)` + x fp16 → y fp16，FP32 累加；nibble 契约（低=k=2b 高=k=2b+1，补码 q∈[-7,7]）；symmetric G=128（scale=amax/7）；**量化+packing 严格计时区外**（`cudalab/int4gemv_quantize.py`，CPU 39/39） |
| 对齐契约 | W_packed 16B ∧ x 16B ∧ K%32==0 → 向量化；不满足 → 标量回退（不拒绝、无静默向量化）；K%128==0 为硬合同（量化期 + host 元数据期双重拒绝）；负例含 2 例对齐回退 bit-identical 钉死 |
| 正确性（三层） | 层 A（门）：5 变体各 **50/50**（5 输入模式 × 9 形状/边界），固定算术界 TOL_K=2，`max_arith_max_ratio = 0.245053042161`（5 变体一致 = 同一 term 序，层 A 独立核验不依赖交叉等价假设）；层 B（门，CPU 侧）：nibble 互逆 **39/39**；层 C（report-only）：每元素误差 ≤ 0.5·量化步长，normal cos ≥ 0.999（K=4096 深抵消）；负例 **30/30 × 5**（25 reject + 5 pass，含 2 回退位级一致） |
| 三口径 | 分开记录不混用。主目标 4096² streaming µs：baseline API 53.197 / NCU 66.432（dram 25.44%, long_sb 64.2%）；**hx API 20.503 / native w5000 22.208 / NCU 25.176（dram 68.84%）**。一致性：三口径变体排序完全一致（hx < rowtile4 < rowtile8 < vec16_row < baseline）；API vs native regime 差（streaming W L2-cold vs 连续循环部分 L2-warm, 3–19% 各代不等）已调查；NCU vs native +13% = observed profiling perturbation（本 workload ~3 µs 量级） |
| 自主优化实验 | **4/4**（paired v2.3 streaming 9r，链式 parent，NCU 证据驱动）：INT4GEMV-0001 `vec16_row` **KEEP 2.000×** [1.9975,2.0105] 9/9（dram 25.44%→48.98%；新瓶颈 lg_throttle 43.9% = x 每行重发）；INT4GEMV-0002 `rowtile4` **KEEP 1.188×** [1.1827,1.1909] 9/9（x 寄存器驻留：内存指令 768→384/行，lg_throttle 43.9%→2.3%；新瓶颈 long_sb 44.3% = W DRAM 延迟）；INT4GEMV-0003 `rowtile8` **NEUTRAL 0.9781×**（统计 SLOWER，0/9）——MLP 假设证伪：long_sb cpi 4.31→3.343 但 dram 62.34%→62.55% 不动，117 寄存器占用率 62.83%→43.09% 塌方（80/117 = 0.684 ≈ 43.09/62.83 = 0.686）；**MLP per thread 不是比 resident thread 更强的 DRAM 杠杆**；INT4GEMV-0004 `rowtile4_hx` **KEEP 1.0869×** [1.0822,1.0887] 9/9（0003 的对照：保持 MLP 4 结构，x float→`__half2` 驻留，ptxas 80→64 寄存器无 spill 且转换未 hoist → occ 62.83%→82.78%，在飞 W 字节 +24%，dram 62.34%→68.84%；三口径同向 1.087/1.092/1.095×）→ **政策 incumbent = `int4gemv_rowtile4_hx`**（链：baseline 51.7 → vec16_row 25.721 → rowtile4 21.600 → hx 19.894，累计 2.599×） |
| 三代对比（fresh 同 session） | @4096² streaming fp16，2026-09-22：FP16 `gemv_vec4_row` 59.977 / INT8 `qgemv_vec16_row` 31.931 / **INT4 hx 20.503 µs**（API；native 59.673/31.486/22.208；NCU 64.24/36.432/25.176 µs，dram 89.3/86.55/68.84%）。**INT4 vs INT8 = 1.557×（理论 2× 的 78%，未接近 2×）**；vs FP16 = 2.925×；INT8 vs FP16 = 1.878×（v0.6 结论 1.90× 的 fresh 复测确认）。缺口两层视图（报告 §8，全部可从 raw 复算）：（1）algorithmic 视图——逻辑 IO 减半 1.940×（W-only 2.0×）= 理论流量削减，不等同于实测 speedup；（2）measured NCU 物理视图——物理流量削减 **1.7963×**（`dram__bytes.sum` 4-launch 均值 18,435,144 B vs 10,262,584 B，raw CSV `profiles/int4gemv/gen3_pipe_*.csv`）× 物理带宽比 **0.8056×**（506.0 vs 407.6 GB/s，bytes÷NCU 主 pass duration 36.432/25.176 µs）= NCU **1.4471×**（= 36.432/25.176 µs，同口径恒等式）；**API 1.557× 单独报告**（不同口径，不用 NCU 分解精确解释）。物理带宽差距候选：(a) 实测流量超逻辑 18.4%（+1.59 MB，实测事实；**归因为假设非结论**——与 x 跨 block 重读 / W streaming L2 再取假设一致，亦可能含 transaction/cache-line 额外流量，需控制实验确认）；(b) 整数线程指令 46.33M vs INT8 38.93M = +19%（每 product 2.76 vs 2.32，nibble 解包 + group scale 索引；指令数为事实，机制为假设）；short_scoreboard 仅 4.5% → **group scale 查找不是瓶颈**（单 group 引理 g=v>>2）。三代 NCU 值均为 v0.7 fresh pass，归档 `profiles/gen3_v0.7/`（v0.7.1 已把 `profiles/gemv|qgemv/` 历史路径恢复为 v0.5/v0.6 发布原值） |
| 全矩阵 | 5 形状 × {hot,streaming} × 5 变体 × 9 轮（tag `v0.7`）：hx **6/10 格 SIGNIFICANT_WINNER**（(4096,1024)/(4096,4096)/(4096,11008) 双 mode, 1.054–1.166×）、**9/10 格最低中位数**（唯一例外 (1024,4096) hot: rowtile4 低 0.31%）；(11008,4096) 双 mode +4.7% < 5% 政策带 → NEUTRAL → NO_UNIQUE_WINNER；(1024,4096) 双 mode rowtile4≈hx ±0.3% 打平（K=1024 nvec=32 < 128 线程平地区域）→ NO_UNIQUE_WINNER；单一 hx 覆盖全部 5 形状 |
| 回归 / 隔离审计 | `experiments/regression/v0.7/`（`scripts/regression_v07.py` 可复现，append-only）：int4gemv 5 变体 50/50 + 30/30 全绿（隔离集空，机制保留）；gemv 4 正常变体 100/100 + 24/24；qgemv 5 变体 50/50 + 29/29；`quarantine_audit.json` 3 算子绑定审计零泄漏（gemv_splitk4 仅 all_variants 在列；softmax_hsplit2 与本分支算子集无关，以 main 上 v0.5/v0.6 审计为准） |
| 独立 review | 2 个独立 subagent（CUDA correctness + Benchmark methodology）——结论与逐条处置见报告 §10 |
| git | 分支 `v0.7-int4-gemv`（10 commit）：2933d87（算子+量化+正确性+负例+baseline）→ bf6aabb（0001）→ 52856c6（0002）→ 5dbfeea（0003）→ fed572f（0004）→ 506da9a（全矩阵+winners）→ b2580d6（三代测量）→ c4aa04c（回归）→ (末) 最终报告+README/STATUS+两路 review 处置+gen3 pipe CSV 归档；v0.7.1 merge 修复轮（外部 review 4 项，2 commit：fix 恢复历史 profile 不可变性 + docs 更正 INT4 物理流量归因/分解/Turing backlog 措辞，无 GPU 重跑）；**不 merge main、不 force push**，push 后 STOP 等外部 review |
| 未做 / v0.8 backlog | 结构性 INT4 优化收尾（收益递减 2.0→1.188→1.087×，MLP 方向已证伪）；v0.8 候选（报告 §11 Q7）：（a）+18.4% DRAM 流量冗余——归因未证实（假设：x 跨 block 重读 / W streaming L2 再取，亦可能含 transaction 开销）。**当前硬件 Turing sm_75 不支持 Ampere+ 的 cudaAccessPolicyWindow / persisting-L2** → 该项 = Turing 兼容 cache 行为调查：区分 x refetch vs transaction 开销、NCU 检查 load/cache 行为、如有依据再考虑 PTX/cache-policy hint（期望 5–10%，不保证）；（b）解包指令开销（编译器级，<5%）；（c）K=1024 线程利用率（128 线程中 96 空转，新结构）；**建议启动 CUDALM 集成**，以 `int4gemv_rowtile4_hx` 为交付内核 |
